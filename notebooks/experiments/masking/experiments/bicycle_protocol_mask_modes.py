...
DATA_PATH = Path("/data/toulouse/bicycle/notebooks/experiments/masking/data/model_runs/experiments/mask_modes")
...
iterator1=[True, False]
iterator2 = [0, 0.5, 1.0, 2.0, 5.0]
for n1 in iterator1:
    for n2 in iterator2:
        for r in range(replicates):
            ...
            data_sem="pert_cif" if n2 == "create_data_scMultiSim" else "linear"

            # scMultiSim
            scMultiSim_path = Path("/data/toulouse/bicycle/notebooks/experiments/masking/scMS.R")
            data_id = "data_run000"
            ...
            if data_source == "create_data":
                ...
                model_init_tensors = {}
                if masking_loss or use_hard_mask:
                    ...
                    if use_hard_mask:
                        hard_mask = (bayes_prior>0).astype(float)
                        hard_mask*= (np.ones(hard_mask.shape)-np.diag(np.ones(hard_mask.shape[0])))
                ...
                masking_parameters = {}
                create_mask = False
                if masking_mode != None and use_atac_mask:
                    create_mask = True
