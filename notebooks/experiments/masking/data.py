...
def create_data(
    ...
    **graph_kwargs,
    ...
    Returns:
        tuple: (
            gt_dyn, (torch.Tensor) Represents the ground truth grn, scaled from -1 to 1 with shape (n_gene x n_genes).
            intervened_variables, (torch.Tensor) Represents the ground truth of interventions in binary with shape (n_samples x n_genes)
            samples, (torch.Tensor) The scRNA count matrix with dimensions (n_samples x n_genes).
            gt_interv, (torch.Tensor) Binary matrix showing the possible intervention contexts with 1 as intervened (genes x contexts)
            sim_regime, (torch.Tensor) First n_samples_control are 0, after that random permutation of range(n_contexts_w_ctrl) for each n_samples_per_perturbation
            beta, (torch.Tensor) Ground truth grn (genes x genes)
            )
    ...
    sim_regime_ctrl = torch.zeros((n_samples_control,), device=device) #create control perturbation regime as zeros for each control sample
    n_contexts_list = train_gene_ko + test_gene_ko # create list of intervention contexts
    n_contexts_list_w_ctrl = [""] + n_contexts_list # add non targeting control to interventions

    sim_regime_pert = torch.arange(1, 1 + len(n_contexts_list), device=device).repeat_interleave(
        n_samples_per_perturbation
    ) # create sim_regime pert as arange of n_contexts_w_ctrl repeated for each sample

    # Shuffle all tensor elements randomly
    sim_regime_pert = sim_regime_pert[torch.randperm(len(sim_regime_pert))] #=gt_interv
    ...
    # which context (including control)
    gt_interv = torch.zeros((N, len(n_contexts_list) + 1), device=device)

    # Gene i is intervened in regime p (genes as rows contexts as cols)
    for idx, p in enumerate(n_contexts_list_w_ctrl):
        # Empy string represents control
        if p == "":
            continue
        elif "," in p:
            # Comma separated values represent multiple interventions (test_gene_ko) -> add 1 in both genes rows
            for pp in p.split(","):
                gt_interv[int(pp), idx] = 1
        else:
            gt_interv[int(p), idx] = 1

    intervened_variables = gt_interv[:, sim_regime].transpose(0, 1) # tensor (samples x genes) showing which genes are perturbed in which samples
...
def create_loaders(
    ...
    **kwargs,
    ...
    """
    samples_interventions = sim_regime.to(torch.int64)

    if len(test_gene_ko) > 0: # Deprecated
        raise DeprecationWarning("test_gene_ko deprecated! Use validation_size > 0 instead")
        # No intervention must be in train and test sim regimes:
        if len([x for x in train_gene_ko if x in test_gene_ko]) > 0:
            raise ValueError("train and test gene knock-outs must be disjoint")
        ...
        # Add dataloader only containing val data
        if validation_size > 0:
            train_idx, validation_idx = train_test_split(
                np.arange(len(samples)),
                test_size=validation_size,
            ...
            # train dataset also contains validation data
            train_dataset = torch.utils.data.TensorDataset(
                samples,
                samples_interventions,
