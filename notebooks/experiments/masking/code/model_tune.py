"""
Modified script of the bicycle model for tuning the hyperparameters. In this script the validation step takes in test_data 
and reports a nll of unseen perturbations to ray.
...
class OmegaIterative(pl.LightningModule):
    ...
    def lyapunov_rhs(self):
        ...
        return self.sigma @ self.sigma.transpose(0, 1)
...
class BICYCLE(pl.LightningModule):
    ...
    def predict_perturbation(
        ...
        **kwargs,
        ...
        sigma_p = math.sqrt(2.0) * torch.tensor(
            target_std, device=self.device).to(dtype=self.sigma.dtype)
        alpha_p = torch.tensor(target_mu, device=self.device).to(dtype=self.alpha.dtype)

        gt_interv_orig = self.gt_interv

        self.gt_interv = torch.zeros((self.n_genes, 1), device=self.device)
        ...
        trainer = pl.Trainer(
            ...
            accelerator="cuda" if str(self.device).startswith("cuda") else "cpu",
            devices= "auto", #if str(self.device).startswith("cuda") else 1,
        ...
        omega = omega_model.omega.detach().to(self.device)
        B = omega_model.B
        # get z_bar
        z_bar = torch.linalg.solve(B, alpha).to(self.device)
    ...
    def evaluate(
        ...
        if compute_nll:
            ...
            if not (cat==2).all():
                raise TypeError("Data contains non test data!")

            gene_idxs = torch.arange(test_samples.shape[1], device=self.device)
            target_mu = kwargs.get("target_mu", torch.median(self.alpha_p.detach()))
    ...
    def evaluate_batch(
            ...
            dist = None,
            use_loss_func = False,
            compute_class_metrics = False,
            **kwargs,
            ):

        
        # compute the nll for unseen perturbations
        test_samples, test_sim_regime, test_z_idxs, cat = batch
        print("test_samples.size: ",test_samples.size())
        ...
        if not (cat==1).all():
            raise TypeError("Data contains non validation data!")
