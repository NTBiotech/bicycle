...
def generate_data(
    ...
    if create_mask:
        ...
        if mask.shape != beta.shape:
            for n, (m, b) in enumerate(zip(mask.shape, beta.shape)):
                ...
                if m > b:
                    print("mask is bigger than grn! Check if RNA trimming was performed.")
                    if n:
                        mask = mask[:,:b-m]
                    else:
                        mask = mask[:b-m]
    ...
    if verbose:
        ...
        # for each condition
        if len(contexts) != 0 and n_samples_per_pert > 0:
            fig, ax = plt.subplots(ncols=len(contexts)//2+1,
                                   nrows=2)
            ax = ax.flatten()
            ...
            for n, context in enumerate(gt_interv.T[1:].astype(bool)):
                ...
                p_values = false_discovery_control(np.array(p_values).astype(float))
            ...
            fig.set_figwidth((len(contexts)+1))
            fig.show()
