...
class BICYCLE(pl.LightningModule):
    def __init__(
        ...
        Args:
            ...
            gt_interv (torch.Tensor): Ground truth intervention matrix (genes x interv_contexts.unique()).
                Encodes in bool which gene is perturbed in which context.
            ...
            - DataLoaders for training this class have to contain:
                1. scRNA data matrix (samples x genes)
                2. sim_regime: tensor containing the context number to index gt_interv
                3. sample_indexes: tensor with range(len(train_set)) to index the corresponding learned latent representation
        ...
        # configure validation_step_outputs for early stopping
    ...
    def training_step(self, batch, batch_idx):
        ...
        samples, sim_regime, sample_idx = batch


        # If samples is empty, we skip the training step
        if (len(samples) == 0) & self.training:
            return None
        ...
        # train latents for validation set too
        if not self.training:
            self.train(True)
        # KL Divergence & NLL Loss
        if self.use_latents:
            mvn = self.get_mvn_normal(B, alphas, sim_regime, sigmas)
            z_kl = self.compute_kl_divergence_loss(
                mvn, sample_idx, samples, sim_regime
            )
            neg_log_likelihood = self.compute_nll_loss(
                samples, sample_idx, sim_regime
            )
            self.log(f"{prefix}_kl", z_kl, **kwargs)
            self.log(f"{prefix}_nll", neg_log_likelihood, **kwargs)
            self.log(
                f"{prefix}_sigma_min",
                torch.diagonal(mvn.covariance_matrix, dim1=-2, dim2=-1).min().detach(),
                **kwargs,
            )
            self.log(f"{prefix}_alpha_min", alphas.min().detach(), **kwargs)
            self.log(f"{prefix}_alpha_max", alphas.max().detach(), **kwargs)

            if self.covariates is not None and self.training:
            ...
            mvn = self.get_mvn_normal(B, alphas, sim_regime, sigmas)
            neg_log_likelihood = self.compute_nll_loss(
                samples, sample_idx, sim_regime, mvn
            )
            z_kl = 0

            self.log(f"{prefix}_nll_train", neg_log_likelihood, **kwargs)
            self.log(
                f"{prefix}_sigma_min",
                torch.diagonal(mvn.covariance_matrix, dim1=-2, dim2=-1).min().detach(),
                **kwargs,
