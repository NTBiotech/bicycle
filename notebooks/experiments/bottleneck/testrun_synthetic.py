...
callbacks = [
    ...
    GenerateCallback(
        MODELS_PATH.joinpath(run_id, "generatecallback", "during.png"),
...
if CHECKPOINTING:
    callbacks.append(
        CustomModelCheckpoint(
            dirpath=os.path.join(MODELS_PATH, run_id, "customcheckpoint"),
            filename="{epoch}",
            save_last=True,
            save_top_k=1,
    ...
    callbacks.append(MyLoggerCallback(dirpath=os.path.join(MODELS_PATH, run_id, "mylogger")))
...
if model.use_latents and n_epochs_pretrain_latents > 0:
    ...
    pretrain_callbacks = [
        RichProgressBar(refresh_rate=1),
        GenerateCallback(
            str(MODELS_PATH.joinpath(run_id, "generatecallback","pretrain_during.png")),
            plot_epoch_callback=plot_epoch_callback,
            true_beta=beta.cpu().numpy(),
        ),
    ]
    ...
    pretrain_callbacks.append(MyLoggerCallback(dirpath=os.path.join(MODELS_PATH, run_id, "mylogger")))
...
plot_training_results(
    ...
    scale_spectral = model_scale_spectral,
    scale_lyapunov = model_scale_lyapunov,
    file_name_plot = PLOTS_PATH.joinpath(run_id, "last.png"),
    callback=True,
    labels=None,
)
