...
def move_to(x, device):
    """Wrapper for the torch.tensor.to() method."""
    if x.device == device:
        globals()["not_move_to_count"] = globals().get("not_move_to_count", 0) + 1
        return x
    else:
        globals()["move_to_count"] = globals().get("move_to_count", 0)+1
        return x.to(device)
...
class BICYCLE(pl.LightningModule):
    ...
    def get_updated_states(self):
        ...
        if self.n_factors == 0:
            ...
            if self.intervention_type == "dCas9":
                betas = iv_a[:, None, :] * move_to(self.beta,self.device) + (1 - iv_a)[
                    :, None, :
                ] * move_to(self.beta_p,self.device)# beta pert updated
            # in case of knockout/knockdown
            elif self.intervention_type == "Cas9":
                betas = iv_a[:, :, None] * move_to(self.beta,self.device) + (1 - iv_a)[
                    :, :, None
                ] * move_to(self.beta_p,self.device)
            else:
                raise NotImplementedError(
            ...
            if self.mask is None:
                if self.intervention_type == "dCas9":
                    ...
                    factor2genes = iv_a[:, None, :] * move_to(self.factor2gene,self.device) + (1 - iv_a)[
                        :, None, :
                    ] * move_to(self.factor2gene_p,self.device)
                    ...
                    # merge factor to gene matrix
                    gene2factors = iv_a[:, :, None] * move_to(self.gene2factor,self.device) + (1 - iv_a)[
                        :, :, None
                    ] * move_to(self.gene2factor_p,self.device)
                    # initialize betas as gene to gene matrix
        ...
        if self.intervention_type == "dCas9":
            alphas = (
                iv_a * self.pos(move_to(self.alpha,self.device))[None, :]
                + (1 - iv_a) * self.pos(move_to(self.alpha_p,self.device))[None, :]
            )
            ...
            alphas = self.pos(move_to(self.alpha,self.device))[None, :].expand(
    ...
    def _get_posterior_dist(self, sample_idx, samples, sim_regime):
        ...
        """
        if self.use_encoder:
            gt_nonzeros = move_to(self.gt_nonzeros,self.device)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if len(samples_test) > 0:
            gt_interv = move_to(self.gt_interv,self.device)
            iv_a = (1 - gt_interv).T
    ...
    def predict_perturbation(
        ...
        move_to(self.gt_interv,self.device)
        ...
        move_to(self.gt_interv,gt_interv_orig.device)
